...
Mat NovelViewUtil::generateNovelViewSimpleCvRemap(
  ...
  Mat novelView;
  remap(srcImage, novelView, warpMap, Mat(), INTER_CUBIC);
  return novelView;
}
...
pair<Mat, Mat> NovelViewGeneratorLazyFlow::renderLazyNovelView(
  ...
  Mat remappedFlow;
  remap(opticalFlow, remappedFlow, warpOpticalFlow, Mat(), INTER_CUBIC);
  ...
  Mat novelView;
  remap(srcImage, novelView, warpComposition, Mat(), INTER_CUBIC);
